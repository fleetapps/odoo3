...
class BaseString(Field[str | typing.Literal[False]]):
    ...
    def get_trans_terms(self, value: str) -> list[str]:
    ...
    def convert_to_column_insert(self, value, record, values=None, validate=True):
        ...
        StoredTranslations(cache_value_dict).validated(record.env, self, check_structure=callable(self.translate))
    ...
    def write(self, records, value):
        ...
        if isinstance(value, dict):
            ...
            if len(value) == 1:
                ...
                if lang in active_langs or lang == 'en_US':
                    self.write(records.with_context(lang=lang), value)
                return
            cache_value_dict = {
                k: self.convert_to_cache(v, records)
                for k, v in value.items()
                if (k in active_langs or k == 'en_US') and
