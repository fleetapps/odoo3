...
class StoredTranslations(dict):
    ...
    def validated(self, env: Environment, field: BaseString, *, check_structure: bool = False, auto_fix: bool = False) -> StoredTranslations | None:
        """Validate stored translations against soft constraints for the translated field"""
        valid_self = self.copy()
        issues = []
        ...
        # en_US must exist
        if not dict.__contains__(valid_self, 'en_US'):  # noqa: PLC2801
            if not auto_fix:
            ...
            if not valid_self:
                return None
            valid_self['en_US'] = next(iter(valid_self.values()))

        # check technical languages which start with '_'
        assert field.translate
        if callable(field.translate):
            for k, v in list(valid_self.items()):
                if k.startswith('_') and not dict.__contains__(valid_self, k[1:]):  # noqa: PLC2801
                    if not auto_fix:
                    ...
                    valid_self[k[1:]] = valid_self['en_US']

        if check_structure:
            assert callable(field.translate)
            translations = {k: valid_self['_' + k] for k in valid_self if not k.startswith('_')}
    ...
    def written(self, env: Environment, field: BaseString, translation_dict: dict[str, ParsedTranslation], *, adapt_close_terms: bool = False, delay_translations: bool = False) -> StoredTranslations | None:
        ...
        valid_self = self.validated(env, field, check_structure=False, auto_fix=True)
