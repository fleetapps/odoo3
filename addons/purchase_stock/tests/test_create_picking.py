...
class TestCreatePicking(ProductVariantsCommon):
    ...
    def test_00_create_picking(self):
        ...
        self.assertRecordValues(receipt_2.move_line_ids, [
            {'product_id': self.product_id_2.id, 'quantity': 1, 'picking_type_id': self.env.ref('stock.picking_type_in').id},
            {'product_id': self.product_sofa_blue.id, 'quantity': 1, 'picking_type_id': self.env.ref('stock.picking_type_in').id}])
        self.assertRecordValues(backorder.move_line_ids, [
            {'product_id': self.product_id_2.id, 'quantity': 4, 'picking_type_id': self.env.ref('stock.picking_type_in').id}])
