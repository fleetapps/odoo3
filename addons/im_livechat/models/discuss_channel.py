...
class DiscussChannel(models.Model):
    ...
    livechat_rating = fields.Selection(selection=[
        ("0", "Not Rated Yet"),
        ("1", "Unhappy"),
        ("3", "Neutral"),
        ("5", "Happy"),
    ], string="Livechat Rating Text", default="0")
    livechat_rating_value = fields.Integer(
        aggregator="avg",
        compute="_compute_livechat_rating_value",
        compute_sql="_compute_sql_livechat_rating_value",
        compute_sudo=True,
    )
    ...
    _livechat_rating_constraint = models.Constraint(
        "CHECK(channel_type != 'livechat' OR livechat_rating IS NOT NULL)",
        "Livechat session should have a rating or '0' if not rated yet."
    )
    ...
    def _compute_livechat_rating_value(self):
        for channel in self:
            channel.livechat_rating_value = (
                int(channel.livechat_rating)
                if channel.livechat_rating
                else False
            )

    def _compute_sql_livechat_rating_value(self, table):
        return SQL(
            "CAST(%(livechat_rating)s AS INTEGER)",
