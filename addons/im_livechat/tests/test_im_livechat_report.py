...
class TestImLivechatReport(TestImLivechatCommon):
    ...
    def test_im_livechat_report_operator(self):
        ...
        rated_channel.flush_recordset(["livechat_rating"])

        self._create_message(rated_channel, self.operator, "2023-03-18 11:00:00")
        result = self.env["im_livechat.report.channel"].formatted_read_group([], aggregates=["rating_value:avg"])
        self.assertEqual(result[0]["rating_value:avg"], 5, "Rating average should be 5, excluding unrated sessions")
