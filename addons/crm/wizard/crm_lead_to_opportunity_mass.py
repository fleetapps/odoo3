...
class CrmLead2opportunityPartnerMass(models.TransientModel):
    ...
    duplicate_count = fields.Integer(string='Duplicated leads', context={'active_test': False},
        compute='_compute_duplicated_lead_ids', store=True, compute_sudo=False)
    # the duplicated_lead_ids field stores all the found duplicates - displayed when using deduplicate
    ...
    # the duplicated_opportunities_ids field only stores matching opportunities, displayed when using convert_and_merge
    ...
    def _compute_lead_count_message(self):
        for convert in self:
            convert.lead_count_message = _(
                ...
                selected_duplicate_count=convert.duplicate_count)
    ...
    def _compute_duplicated_lead_ids(self):
        for convert in self:
            ...
            duplicate_opportunities = self.env['crm.lead']
            for lead in convert.lead_tomerge_ids:
                ...
                if len(duplicate_leads) > 1:
                    ...
                    duplicate_opportunities += duplicate_leads.filtered(lambda l: l.type == 'opportunity')

            convert.duplicate_count = duplicated
            convert.duplicated_lead_ids = all_duplicates.ids
            convert.duplicated_opportunity_ids = duplicate_opportunities.ids

    @api.onchange('duplicated_lead_ids')
    def _onchange_duplicated_lead_ids(self):
        for convert in self:
            if not convert.duplicated_lead_ids:
                convert.name = 'convert'

    @api.onchange('duplicated_opportunity_ids')
    def _onchange_duplicated_opportunity_ids(self):
        for convert in self:
            if not convert.duplicated_opportunity_ids:
                convert.name = 'convert'
    ...
    def _action_convert_and_merge(self):
        ...
        for lead in self.lead_tomerge_ids:
            duplicate_opportunities = self.env['crm.lead']._get_lead_duplicates(
                partner=lead.partner_id,
                email=lead.partner_id and lead.partner_id.email or lead.email_from,
                include_lost=False).filtered(lambda l: l.type == 'opportunity') & self.duplicated_opportunity_ids
