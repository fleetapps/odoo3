...
test("drops blank textual entries", async () => {
    ...
    addBuilderOption(
        class extends BaseOptionComponent {
            ...
            static components = { BuilderList };
            static props = ["*"];
            static selector = ".test-options-target-c";
        }
    );
    await setupHTMLBuilder(`
        <div class="test-options-target-a">a</div>
        <div class="test-options-target-b">b</div>
        <div class="test-options-target-c">c</div>`);

    // allowEmptyText="false" forbidLastItemRemoval="false"
    ...
    // allowEmptyText="true" forbidLastItemRemoval="false"
    ...
    // allowEmptyText="false" forbidLastItemRemoval="true"
    await contains(":iframe .test-options-target-c").click();
    await contains(".we-bg-options-container .builder_list_add_item").click();
    expect(".we-bg-options-container input").toHaveCount(1);
