...
/**
 * @typedef {((args: {isMobileView: boolean}) => ())[]} on_device_view_switched_handlers
...
export class MobilePreviewPlugin extends Plugin {
    ...
    setup() {
        ...
        this.observer = new ResizeObserver(() => {
            console.log("Resize from MobilePreviewPlugin");
            const isMobileView = this.config.isMobileView(this.editable);
            if (this.isMobileView !== isMobileView) {
                this.isMobileView = isMobileView;
                this.trigger("on_device_view_switched_handlers", { isMobileView });
            }
        });
