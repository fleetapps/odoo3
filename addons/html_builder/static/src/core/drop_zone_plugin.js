...
export class DropZonePlugin extends Plugin {
    ...
    getDropRootElement() {
        ...
        if (
            openModalEl &&
            !this.getResource("is_element_hidden_predicates").some((p) => p(openModalEl))
        ) {
            return openModalEl;
        }
    ...
    checkSelectors(el, rootEl) {
        ...
        if (this.getResource("is_element_hidden_predicates").some((p) => p(el))) {
            return false;
        }
    ...
    activateDropzones(
        ...
        const isIgnored = (el) =>
            el.matches(".o_we_no_overlay") ||
            this.getResource("is_element_hidden_predicates").some((p) => p(el));
        let hookEls = [];
