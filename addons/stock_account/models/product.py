...
class ProductProduct(models.Model):
    ...
    def _run_average_batch(self, at_date=None, lot=None, force_recompute=False):
        ...
        for manual_value in last_manual_value_by_product.values():
            ...
            date_to_bisect[product.id] = manual_value.date

        offset = 0
        limit = 50000
        processing = True
        val_qty_by_product = dict()  # product is the key, (total_value, total_qty) is the value
        ...
        while processing:
            ...
            for product in moves_by_product:
                if product.id not in date_to_bisect:
                    continue
                moves = moves_by_product.get(product, self.env['stock.move'])
                index = bisect(moves, date_to_bisect[product.id], key=lambda m: m.date)
                moves = moves[index:]
            ...
            for product, moves in moves_by_product.items():
                ...
                if product.id not in val_qty_by_product:
                    ...
                    value, quantity, average_cost = val_qty_by_product[product.id]
                for move in moves:
                ...
                val_qty_by_product[product.id] = (value, quantity, average_cost)
