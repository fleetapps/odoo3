...
class TestGelatoDelivery(GelatoCommon, DeliveryCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.partner.write({
    ...
    def test_only_generic_carriers_are_available_for_generic_order(self):
        ...
        self.assertTrue(all(carrier.delivery_type != 'gelato' for carrier in standard_carriers))
    ...
    def test_only_gelato_carriers_are_available_for_gelato_order(self):
        ...
        self.assertTrue(all(carrier.delivery_type == 'gelato' for carrier in gelato_carriers))
