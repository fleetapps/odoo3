...
class TestDataRecycle(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.DUModel = cls.env['data_cleaning.unified']
        cls.unified_model = cls.DUModel.create({
        ...
        cls.recycle_model = cls.env['data_recycle.model'].create({
            'name': 'Recycle Test Server',
            'unified_id': cls.unified_model.id,
    ...
    def test_recycle_notification(self):
        ...
        self.unified_model.notify_user_ids = [(4, self.env.ref('base.user_admin').id)]
        old_notif_count = self.env['mail.notification'].search_count([])
        self.DUModel._clean_records(self.unified_model)
        self.unified_model._notify_records()
