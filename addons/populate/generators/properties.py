    ...
    def _next(self, known_vals):
        ...
        # Fallback: User doesn't care about specific values -> Random generation
            ...
            for i in range(self.count):
                prop_type = self.rng.choice(self.allowed_types)
                ...
                if prop_type == 'selection':
                    ...
                    prop['tags'] = [
                        (f't{j}', val, self.rng.randint(1, 11))
...
class PropertyProp(Generator):
    ...
    def __init__(self, prop_type: str, string: str, possible_values: Sequence[str] | None = None, **kwargs):
        ...
        if prop_type == 'selection':
            ...
            prop['tags'] = [
                (f't{i}', val, self.rng.randint(1, 11))
...
def _random_value(prop, rng):
    match prop['type']:
        ...
        case 'char' | 'text':
            return ''.join(rng.choices(ascii_letters + digits, k=12))
        case 'selection':
            options = prop['selection']
            return rng.choice(options)[0]
        case 'tags':
            tags = prop['tags']
            k = rng.randint(0, len(tags))
            return [t[0] for t in rng.sample(tags, k)]
        case 'date':
            return (date.today() - timedelta(days=rng.randint(0, 365))).isoformat()
        case 'datetime':
            return (datetime.now() - timedelta(days=rng.randint(0, 365))).isoformat()
        case _:
            return None
...
class PropertyValue(Generator):
    ...
    def __init__(self, field: 'fields.Properties', **kwargs):
        ...
        super().__init__(field=field, depends=[field.definition_record], **kwargs)
    ...
    def _next(self, known_vals):
        ...
        for prop in definitions:
            ...
            # -> Random generation
            values[prop['name']] = _random_value(prop, self.rng)
