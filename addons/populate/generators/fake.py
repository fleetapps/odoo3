...
# Stable Policy: Only modifying/removing entries is allowed in 'stable', unless urgent
METHODS_BLACKLIST = {
    'boolean',  # from misc: Use scalar.boolean instead
    'image',    # from misc: Depends on libs that might not be present
}
...
def create(method_name: str):
    ...
    if method_name not in KNOWN_METHODS:
        raise ValueError(_(
            "Cannot create a fake generator for unknown method '%s'. "
            "It must be in KNOWN_METHODS (i.e., from an allowed Faker provider).",
            method_name,
        ))
    ...
    class Fake(Generator):
        ...
        def __init__(self, locale='en_US', **kwargs):
            ...
            allowed_params = {'field', 'env', 'rng', 'session', 'valid_fields', 'unique', 'null_frac', 'locale'}
            ...
            def convert(val: str):
                match val.strip().lower():
                    ...
                    case str(_):
                        ...
                        return val.strip()
                    case _:
                        raise RuntimeError(_("Unreachable! All attribute values should be `str` at the source."))
