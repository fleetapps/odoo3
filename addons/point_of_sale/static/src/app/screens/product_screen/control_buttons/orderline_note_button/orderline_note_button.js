...
export class NoteButton extends Component {
    ...
    async setChanges(selectedOrderline, payload) {
        const saved_quantity = selectedOrderline.prep_line_ids.reduce((totalQty, prepLine) => {
            totalQty += prepLine.quantity;
            return totalQty;
        }, 0);

        const qtyToAdd = selectedOrderline.qty - saved_quantity;
        if (saved_quantity > 0 && qtyToAdd > 0) {
