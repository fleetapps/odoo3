...
class PosConfig(models.Model):
    ...
    def _get_pos_graph_data(self):
        ...
        week_ago = today.date() - timedelta(days=6)
        ...
        self.env.cr.execute("""
            SELECT o.config_id,
             ...
             WHERE o.config_id = ANY(%s)
               ...
               AND DATE(o.date_order AT TIME ZONE 'UTC') >= %s
        ...
        for config in self:
            ...
            data = [build_graph_data(today.date() - timedelta(days=i), config_rows.get(today.date() - timedelta(days=i), 0)) for i in range(6, -1, -1)]
