...
class TestDeliveryPickingBatch(common.TransactionCase):
    ...
    def test_auto_batch_carrier_change_after_confirmation(self):
        ...
        carrier = self.local_delivery_carrier
        pick_1, pick_2 = self.env['stock.picking'].create([{
            'name': f"Pick {carrier.name if info[0] else 'with no carrier'}",
            'partner_id': info[1],
        ...
        self.assertEqual(ship_1.batch_id, ship_2.batch_id)
