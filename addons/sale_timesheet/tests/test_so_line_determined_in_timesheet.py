...
class TestSoLineDeterminedInTimesheet(TestCommonSaleTimesheet):
    ...
    def test_sol_determined_when_project_is_task_rate(self):
        ...
        self.assertEqual(timesheet.sale_order_line_id, task.sale_line_id, "The SOL in the timesheet should be the same than the one in the task.")
        ...
        self.assertEqual(timesheet.sale_order_line_id, task.sale_line_id, "The SOL in the timesheet should also change and be the same than the one in the task.")
    ...
    def test_sol_determined_when_project_is_project_rate(self):
        ...
        self.assertTrue(timesheet.sale_order_line_id == task.sale_line_id == self.so.order_line[0], "The SOL in the timesheet should be the same than the one in the task.")
        ...
        self.assertTrue(timesheet.sale_order_line_id == task.sale_line_id == self.so.order_line[1], "The SOL in the timesheet should also change and be the same than the one in the task.")
    ...
    def test_sol_determined_when_project_is_employee_rate(self):
        ...
        employee_user_timesheet._compute_sale_order_line_id()
        self.assertTrue(employee_user_timesheet.sale_order_line_id == self.project_employee_rate.sale_line_employee_ids[0].sale_line_id == self.so.order_line[1], "The SOL in the timesheet should be the one defined in the mapping for the employee user.")
        ...
        self.assertTrue(timesheet.sale_order_line_id == task.sale_line_id == self.so.order_line[2], "The SOL in the timesheet should also change and be the same than the one in the task.")
        self.assertNotEqual(timesheet.sale_order_line_id, employee_user_timesheet.sale_order_line_id, "The SOL in the timesheet done by the employee user should not be the same than the one in the other timesheet in the task.")

        # 6) Change the SOL in the mapping and check if the timesheet conserne by the mapping has its SOL has been changed too.
        mapping.update({'sale_line_id': self.so.order_line[-1].id})
        self.assertTrue(employee_user_timesheet.sale_order_line_id == mapping.sale_line_id == self.so.order_line[-1], "The SOL in the timesheet done by the employee user should be the one defined in the mapping.")
        self.assertNotEqual(timesheet.sale_order_line_id, employee_user_timesheet.sale_order_line_id, "The other timesheet should not have the SOL defined in the mapping.")
    ...
    def test_no_so_line_if_project_non_billable(self):
        ...
        self.assertFalse(timesheet.sale_order_line_id, 'No SOL should be linked in this timesheet because the project is non billable.')
        ...
        # 5) Check the timesheet has no SOL too since the project is non billable.
        self.assertFalse(timesheet1.sale_order_line_id, 'This Timesheet is not billable since it has no task set and the project linked is not billable')
    ...
    def test_tranfer_project(self):
        ...
        # 4) Check if the timesheet contains the same SOL than the task,
        self.assertEqual(timesheet.sale_order_line_id, task.sale_line_id, 'The timesheet should have the same SOL than the task.')

        # 5) Move the task in a non billable project,
        task.write({'project_id': self.project_non_billable.id})

        # 6) Check if the task and timesheet has no SOL.
        self.assertFalse(timesheet.sale_order_line_id, 'No SOL should be linked to the timesheet because the project is non billable')
