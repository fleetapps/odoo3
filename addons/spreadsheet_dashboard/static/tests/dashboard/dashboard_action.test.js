...
test("shows not found error when opening a deleted dashboard from URL", async () => {
    onRpc(
        "/spreadsheet/dashboard/data/999",
        () => new Response("<h1>Not Found</h1>", { status: 404 }),
        { pure: true }
    );
    await createSpreadsheetDashboard({ spreadsheetId: 999 });
    expect(".o-spreadsheet").toHaveCount(0, { message: "It should not display the spreadsheet" });
    expect(".o_spreadsheet_dashboard_action .dashboard-loading-status.error").toHaveText(
        "The requested dashboard could not be found."
    );
    expect(".o_renderer .error").toHaveCount(0, { message: "It should not display a traceback" });
});
