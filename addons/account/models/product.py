            ...
            for company_domain in (
                ...
                for domain in domains:
                    if products := self.env['product.product'].search(
                        ...
                        search_field = domain[0][0]
                        for product in products:
                            if search_field == 'product_tmpl_id':
                                for val_dict in products_vals:
                                    input_name = val_dict.get('name', '').split('\n', 1)[0]
                                    if not input_name:
                                        continue
                                    if input_name.lower() in (product.name or '').lower():
                                        all_products[input_name] = product
                            else:
                                if search_field in ('barcode', 'default_code'):
                                    val = getattr(product, search_field)
                                if val:
                                    all_products[val] = product
        return all_products
