...
class ProductProduct(models.Model):
    ...
    def _retrieve_product(self, name=None, default_code=None, barcode=None, company=None, extra_domain=None):
        ...
        def search_product_by_name_similarity(base_domain):
            """ Returns the first product whose name similarity ratio with the provided name is at least 90%. """
            products_with_similar_name = self.search(
                expression.AND([
                    [('name', 'ilike', name)],
                    base_domain,
                ]),
            )
            for product in products_with_similar_name:
        ...
        for company_domain in (
            ...
            if name and len(name) > 4 and (product := search_product_by_name_similarity(base_domain)):
                return product
