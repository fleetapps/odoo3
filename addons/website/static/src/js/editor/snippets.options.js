...
options.registry.ImageTools.include({
    ...
    async _applyOptions() {
        ...
        if (img && img.dataset.shape && !img.dataset.shapeColors) {
            const shapeName = img.dataset.shape.split("/")[2];
            const shape = this.shapeCache[shapeName];
            if (shape) {
                const defaultColors = Object.values(weUtils.DEFAULT_PALETTE).map((color, i) =>
