    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                // Bootstrap the Google Maps JS API using dynamic library
                // import method. Libraries (e.g. "maps", "places", "marker")
                // are loaded lazily via `google.maps.importLibrary()` at each
                // call site.
