...
describe("Image format/optimize", () => {
    ...
    test("Quality option does not disappear when a shape is applied on the image", async () => {
        ...
        // been applied. The second one correspond to a modified image on which
        // a shape has been applied. The last one corresponds to a case where
        // the mimetype information are not present (this case should not appear
...
test("Correct options appear when clicking on a gif image", async () => {
    await clickOnImgAndCheckOptionsNotDisplayed(testGifImg);
});

test("Correct options appear when clicking on a svg image", async () => {
    await clickOnImgAndCheckOptionsNotDisplayed(testSvgImg);
});

async function clickOnImgAndCheckOptionsNotDisplayed(imgEl) {
    const { waitSidebarUpdated } = await setupWebsiteBuilder(`
        <div class="test-options-target">
            ${imgEl}
        </div>
