...
class StockPicking(models.Model):
    ...
    def write(self, vals):
        ...
        if 'carrier_id' in vals:
            ...
            if not new_carrier.support_pickup_locations and any(picking.carrier_id.support_pickup_locations for picking in self):
                pickings_with_pickup = self.filtered(lambda so: so.carrier_id.support_pickup_locations)
    ...
    def action_confirm(self):
        ...
        """
        if any(picking.is_pickup_required and picking.partner_id.pickup_delivery_method_id.id != picking.carrier_id.id for picking in self):
    ...
    def set_pickup_location(self, pickup_location_data):
        ...
        if self.is_pickup_required:
            ...
            parent_location = self.partner_id.parent_id if self.partner_id.pickup_delivery_method_id else self.partner_id
            address = self.env['res.partner']._address_from_json(pickup_location_data_json, parent_location, pickup_delivery_method_id=self.carrier_id.id)
