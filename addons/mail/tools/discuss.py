...
class StoreVersionInternal:
    ...
    def __init__(self):
        # Maps model names to record IDs to the set of field names written.
        self.written_fields_by_record = defaultdict(lambda: defaultdict(OrderedSet))
        self.context = {
            "manager": self,
    ...
    def _mark_fields_as_written(self, record, fnames, with_orm=True):
        ...
        """
        self.written_fields_by_record[record._name][record.id].update(fnames)
        if with_orm:
            self.written_fields_by_record[record._name][record.id].update(
    ...
    def _get_formatted_version(self, env):
        ...
        return {
            ...
            "written_fields_by_record": {
                ...
                for model, recs in self.written_fields_by_record.items()
            },
        }
...
def store_version(func=None, *, make_json_response=False):
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            ...
            if make_json_response:
                req.make_json_response(response)
            return response
        return wrapper
