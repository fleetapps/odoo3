...
import { loadEmoji } from "@web/core/emoji_picker/emoji_loader";
...
export async function start(options) {
    ...
    // Note that `loadEmoji` cannot be called before setting up the env because
    // it depends on translations being loaded.
    await Promise.all([mountWithCleanup(WebClient, { env, target }), loadEmoji()]);
