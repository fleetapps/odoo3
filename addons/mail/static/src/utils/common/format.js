...
import { emojiLoader, loadEmoji } from "@web/core/emoji_picker/emoji_loader";
...
export function decorateEmojis(content) {
    ...
    for (let i = 0; i < nodes.snapshotLength; i++) {
        ...
        setElementContent(
            span,
            htmlReplaceAll(node.textContent, emojiLoader.emojiRegex, (codepoints) =>
                markup(
                    `<span class="o-mail-emoji" title="${htmlFormatList(
                        emojiLoader.emojiMap.get(codepoints).shortcodes,
                        { style: "unit-narrow" }
