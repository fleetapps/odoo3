...
from odoo.addons.website_sale.controllers.payment import (
    PaymentPortal as PaymentController,
)
    ...
    def test_final_payment_step_check_all_checkout_steps(self):
        ...
        with (
            patch(
                ...
                return_value=None,
            ) as check_post_shop_cart_step,
            patch(
                'odoo.addons.website_sale.controllers.main.WebsiteSale._check_post_shop_address_step',
                return_value=None,
            ) as check_post_shop_address_step,
            patch(
                'odoo.addons.website_sale.controllers.main.WebsiteSale._check_post_shop_checkout_step',
                return_value=None,
            ) as check_post_shop_checkout_step,
        ):
            self.url_open('/shop/payment')

            check_post_shop_cart_step.assert_called_once()
            check_post_shop_address_step.assert_called_once()
            check_post_shop_checkout_step.assert_called_once()
            self.assertEqual(
                check_post_shop_checkout_step.call_args[1],
                {},
            ...
            self.assertEqual(check_post_shop_cart_step.call_count, 2)
            self.assertEqual(check_post_shop_address_step.call_count, 2)
            self.assertEqual(check_post_shop_checkout_step.call_count, 2)
            self.assertEqual(
                check_post_shop_checkout_step.call_args[1],
