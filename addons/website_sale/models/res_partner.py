...
class ResPartner(models.Model):
    ...
    pickup_delivery_method_id = fields.Many2one('delivery.carrier', ondelete='cascade')  # The delivery method that generated this pickup location.
    ...
    def _get_delivery_address_domain(self):
        return super()._get_delivery_address_domain() & Domain('pickup_delivery_method_id', '=', False)

    def _is_address_usable(self):
        """ Override to prevent using pickup locations as regular delivery addresses. """
        return super()._is_address_usable() and not self.pickup_delivery_method_id
    ...
    def _address_from_json(self, json_location_data, parent_id, pickup_delivery_method_id=False):
        ...
        :param str pickup_delivery_method_id: The type of the delivery method that generated this pickup location.
    ...
    def write(self, vals):
        ...
        for key in ['phone', 'email']:
            if key in vals:
                for partner in self:
                    children = self.env['res.partner'].with_context(active_test=False).search([('parent_id', '=', partner.id), ('type', '=', 'delivery'), ('pickup_delivery_method_id', '!=', False)])
