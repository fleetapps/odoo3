...
class DeliveryCarrier(models.Model):
    ...
    support_pickup_locations = fields.Boolean(compute='_compute_support_pickup_locations')

    @api.depends('delivery_type')
    def _compute_support_pickup_locations(self):
        for carrier in self:
            carrier.support_pickup_locations = hasattr(carrier, f'{carrier.delivery_type}_use_locations') and getattr(carrier, f'{carrier.delivery_type}_use_locations')
