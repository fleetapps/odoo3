import { BasePrinter } from "@point_of_sale/app/utils/printer/base_printer";
import { logPosMessage } from "@point_of_sale/app/utils/pretty_console_log";

const CONSOLE_COLOR = "#28ffeb";

/* global SUNMI */
export class SunmiPrinterAdapter extends BasePrinter {
    setup({ fallbackPrinter } = {}) {
        super.setup(...arguments);
        this.sunmiPrinter = new SUNMI();
        this.fallbackPrinter = fallbackPrinter;
        const { BitmapStyle } = this.sunmiPrinter.class;
        this.BitmapStyle = BitmapStyle;
    }

    async isAvailable() {
        return new Promise((resolve) => {
            try {
                const url = "ws://localhost:7070/ws";
                const ws = new window.WebSocket(url);
                ws.onopen = function () {
                    ws.close();
                    resolve(true);
                };
                ws.onerror = function () {
                    resolve(false);
                };
            } catch (error) {
                logPosMessage(
                    "SunmiPrinterAdapter",
                    "isAvailable",
                    "Error checking printer availability: " + error.message,
                    CONSOLE_COLOR,
                    [error]
                );
                resolve(false);
            }
        });
    }

    async launchPrintService() {
        await this.sunmiPrinter.launchPrinterService();
        await this.sunmiPrinter.init();
    }

    async printerStatus() {
        try {
            return await this.sunmiPrinter.printer.queryApi.getStatus();
        } catch (error) {
            logPosMessage(
                "SunmiPrinterAdapter",
                "printerStatus",
                "Failed to get printer status: " + error.message,
                CONSOLE_COLOR,
                [error]
            );
            return { status: "UNKNOWN" };
        }
    }

    /**
     * @override
     */
    processCanvas(canvas) {
        return canvas.toDataURL("image/jpeg");
    }

    /**
     * @override
     */
    async printReceipt(receipt) {
        const res = await this.printerStatus();
        // We only use the fallback printer if the printer is not connected or not powered on
        if (res.status.slice(1, -1) !== "READY" && this.fallbackPrinter) {
            return await this.fallbackPrinter.printReceipt(receipt);
        }
        return await super.printReceipt(receipt);
    }

    /**
     * @override
     */
    async sendPrintingJob(img) {
        try {
            const res = await this.printerStatus();
            if (res.status.slice(1, -1) !== "READY") {
                return { result: false, errorCode: res.status, canRetry: true };
            }
            await this.sunmiPrinter.printer.lineApi.printBitmap(
                img.replace(/^data:.+;base64,/, ""),
                this.BitmapStyle.getStyle().setAlign("CENTER")
            );
            this.sunmiPrinter.printer.lineApi.autoOut();
            return { result: true };
        } catch (error) {
            logPosMessage(
                "SunmiPrinterAdapter",
                "sendPrintingJob",
                "Printing job failed: " + error.message,
                CONSOLE_COLOR,
                [error]
            );
            return { result: false, errorCode: error.message, canRetry: true };
        }
    }
}
