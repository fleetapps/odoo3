...
class TestUBLGr(AccountTestInvoicingCommon):
    ...
    def _mark_as_sent(self, invoice, mark):
    ...
    def test_export_greek_invoice(self):
        ...
        self._mark_as_sent(invoice, mark=400001958334210)
    ...
    def test_export_greek_credit_note(self):
        ...
        self._mark_as_sent(refund, mark=123456789)
    ...
    def test_credit_note_preceding_invoice_reference(self):
        """BT-25: Credit note must carry a BillingReference to the original invoice."""
        original_invoice = self._create_gr_invoice(l10n_gr_edi_inv_type='1.1')
        self._mark_as_sent(original_invoice, mark=400001958317039)

        refund = self._create_gr_invoice(move_type='out_refund', l10n_gr_edi_inv_type='1.2')
        refund.reversed_entry_id = original_invoice.id
        self._mark_as_sent(refund, mark=400001958317040)

        root = etree.fromstring(self._export_ubl_gr_invoice(refund))
