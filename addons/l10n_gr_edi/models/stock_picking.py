...
class StockPicking(models.Model):
    ...
    @api.onchange('is_greek_company', 'company_id', 'id')
    ...
    @api.onchange('is_greek_company', 'partner_id', 'id')
        ...
        for line_no, line in enumerate(self.move_ids, start=1):
            if not line.l10n_gr_edi_measurement_unit:
                errors[f'l10n_gr_edi_{line_no}_missing_uom'] = {
                    'message': _("myDATA does not accept the unit on line %s, please select between 'Units', 'kg', 'L', 'm', 'm²' or 'm³'", line_no),
                }
        return errors
    ...
    def _l10n_gr_edi_attach_report_picking(self):
        ...
        attachment = self.env['ir.attachment'].create({
            ...
            'type': 'binary',
            'datas': base64.b64encode(report_picking),
    ...
    def l10n_gr_edi_try_send_delivery_note(self):
        self.ensure_one()
        if error := self._l10n_gr_edi_get_pre_error_dict():
            self._l10n_gr_edi_create_error_document(self, {'error': utils.get_pre_error_string(error)})
        else:
