...
class ReportMrpReport_Mo_Overview(models.AbstractModel):
    ...
    def _contextualized_production(self, production):
        if production.subcontractor_id:
            ...
            return production.with_context(warehouse_id=resupply_warehouse.id, documents=documents)
    ...
    def _get_report_data(self, production_id):
        ...
        if summary['model'] == 'mrp.production':
            ...
            # check 'master' manufacturing order (the one from which the Overview was opened)
        ...
        if len(result) == 1:
            ...
            if summary['model'] == 'to_order':
                ...
                if bom and bom.type == 'subcontract':
                    # subcontracted component not yet ordered: a line to replenish exists
                ...
                if subcontracted_mo:
                    ...
                    # draft purchase order, a line with purchase order exists
                    # add a line with the cost of components
                    result.append(self._make_components_line(summary, product, quantity, unit, bom, level, current_index))
        return result
