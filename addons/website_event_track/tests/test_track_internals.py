...
class TestSplitTrack(TestEventOnlineCommon):

    def test_split_track_by_days_spanning_midnight(self):
        """A track crossing midnight should split at 00:00 local time."""
        ...
        result = EventTrackController()._split_track_by_days(track, local_tz)

        buckets = list(result.items())
    ...
    def test_split_track_by_days_spanning_multiple_days(self):
        track = self.env['event.track'].create({
            'name': 'Multi Day Track',
            'event_id': self.event_0.id,
            'date': datetime(2020, 7, 6, 20, 0, 0),
            'duration': 50.0,
        })
