...
test("sortBy: getter", () => {
    ...
    expect(sortBy([{ x: 2 }, { x: 1 }, { x: 5 }], getter)).toEqual([{ x: 1 }, { x: 2 }, { x: 5 }]);
...
test("ensureArray", () => {
    const arrayRef = [];
    expect(ensureArray(arrayRef)).toBe(arrayRef);
...
test("shallowEqual: simple valid cases", () => {
    expect(shallowEqual([], [])).toBe(true);
    expect(shallowEqual([1], [1])).toBe(true);
    expect(shallowEqual([1, "a"], [1, "a"])).toBe(true);
});

test("shallowEqual: simple invalid cases", () => {
    expect(shallowEqual([1], [])).not.toBe(true);
    expect(shallowEqual([], [1])).not.toBe(true);
    expect(shallowEqual([1, "b"], [1, "a"])).not.toBe(true);
});

test("shallowEqual: arrays with non primitive values", () => {
    const obj = { b: 3 };
