...
class SmsComposer(models.TransientModel):
    ...
    def _compute_recipients(self):
        for composer in self:
            ...
            if records:
                res = records._mail_get_recipients_phone_info(force_field=composer.number_field_name, partner_fallback=not composer.comment_single_recipient)
    ...
    def _compute_recipient_single_stored(self):
        for composer in self:
            ...
            res = records._mail_get_recipients_phone_info(force_field=composer.number_field_name, partner_fallback=not composer.number_field_name)
    ...
    def _compute_recipient_single_non_stored(self):
        for composer in self:
            ...
            if not records or not composer.comment_single_recipient:
                ...
                continue
            records.ensure_one()
            res = records._mail_get_recipients_phone_info(force_field=composer.number_field_name, partner_fallback=True)
    ...
    def _prepare_recipient_values(self, records):
        recipients_info = records._mail_get_recipients_phone_info(force_field=self.number_field_name)
