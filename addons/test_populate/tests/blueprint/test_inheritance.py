...
class TestBlueprintInheritance(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.base_blueprint = self.env['populate.blueprint'].create({
            'name': 'Base Blueprint',
            'definition_xml': '''
                <blueprint>
    ...
    def test_inherit_modify_model_attributes(self):
        child_blueprint = self.env['populate.blueprint'].create({
            ...
            'parent_id': self.base_blueprint.id,
            'definition_xml': '''
                <data>
...
class TestInheritanceChaining(TransactionCase):
    ...
    def test_inherit_recursion_prevention(self):
        ...
        with self.assertRaises(ValidationError):
            self.base_blueprint.write({'parent_id': child_blueprint.id})

    def test_inherit_self_reference_prevention(self):
        with self.assertRaises(ValidationError):
            self.base_blueprint.write({'parent_id': self.base_blueprint.id})
