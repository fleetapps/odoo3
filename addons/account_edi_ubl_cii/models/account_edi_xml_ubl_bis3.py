...
class AccountEdiXmlUBLBIS3(models.AbstractModel):
    ...
    * Official doc for EHF Billing 3.0 is the OpenPeppol BIS 3 doc +
    ...
    def _import_bis3_invoice_add_partner_bank(self, tree, collected_values):
        ...
        if bank_details:
            partner_bank = self._import_partner_bank(invoice, bank_details)
    ...
    def _import_bis3_invoice_line_price_unit_quantity_discount(self, line_tree, collected_values, line_collected_values):
        ...
        if line_extension_amount:
            line_extension_amount *= file_document_sign
            ...
            if base_amount := price_allowance_values.get('base_amount'):
                ...
                # For more reference, check `test_partial_import_invoice_line_price_unit_quantity_6.xml`
                if price_amount_str and (price_amount := float(price_amount_str)) != 0.0:
                    # When quantity is explicitly 0, we rewrite them by the values of line_extension_amount and price_amount (if exist)
                    price_unit = price_amount
                    quantity = line_extension_amount / price_unit
                else:
                    # Otherwise, force reset quantity to 1 and price_unit to the line_extension_amount
                    price_unit = line_extension_amount + total_allowance - total_charges
                    quantity = 1.0
            else:
        ...
        line_collected_values['price_unit'] = abs(price_unit)
        ...
        for charge in line_collected_values['charges']:
            if charge['reason_code'] == 'AEO':
                ...
                retrieve_params = {
                    ...
                    'extra_domain': [('name', '=', charge['reason'])] if charge.get('reason') else [],
                }
                tax = AccountTax._retrieve_tax(**retrieve_params)

                if not tax and retrieve_params['extra_domain']:
                    retrieve_params.pop('extra_domain')
                    tax = AccountTax._retrieve_tax(**retrieve_params)
            ...
            extra_charges_lines.append({
                'name': f"{charge['reason_code'] or ''} {charge['reason']}",
                'quantity': 1.0,
    ...
    def _import_bis3_add_invoice_lines(self, tree, collected_values):
        ...
        for xpath in ('./{*}InvoiceLine', './{*}CreditNoteLine'):
            for line_tree in tree.iterfind(xpath):
                ...
                # Do not add empty amount lines, otherwise continue importing
                if line_collected_values['price_unit'] == line_collected_values['discount'] == 0.0:
                    continue
    ...
    def _import_invoice_ubl_cii(self, invoice, file_data, new=False):
        ...
        for line_collected_values in collected_values['invoice_lines_values']:
            ...
            if account := line_collected_values.get('account'):
                invoice_line_values['account_id'] = account
        ...
        with invoice._disable_recursion({'records': invoice}, 'ignore_quantity_precision'):
        ...
        if tax_total_values['is_complete']:
            ...
            def grouping_function(_base_line, tax_data):
                return tax_data and tax_data['tax'] in reverse_tax_mapping and reverse_tax_mapping[tax_data['tax']]
        ...
        # However, in 18.0, we don't have `extra_tax_data`, and thus have to manually readjust with the values from `_prepare_tax_lines`
        ...
        with invoice._check_balanced(container), invoice._sync_dynamic_lines(container):
            invoice.with_context(dynamic_unlink=True).line_ids = line_ids_commands

        # Attempts to create a PDF attachment when the XML file doesn't provide one
        if invoice.is_purchase_document() and (attachment := self._generate_additional_attachment(invoice)):
            invoice._message_set_main_attachment_id(attachment, force=True, filter_xml=False)

        # Allow localizations to add their own field writes on the imported invoice
        self._import_fill_invoice(invoice, tree, invoice.direction_sign)

    def _import_fill_invoice(self, invoice, tree, qty_factor):
        # OVERRIDES ubl_20, to override
        pass
