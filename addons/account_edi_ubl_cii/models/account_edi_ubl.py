...
from odoo.tools import formatLang, frozendict, html_escape, pdf
...
class AccountEdiUBL(models.AbstractModel):
    ...
    def _import_ubl_invoice_add_customer_values(self, collected_values):
        ...
        tree = collected_values['tree']
        party_node = tree.find("./{{*}}{party_tag}]/{{*}}Party]".format(party_tag=party_tag))
        ...
        # Peppol EAS/Endpoint.
        if node := self.get_node(party_node, "./{*}EndpointID"):
    ...
    def _import_ubl_create_missing_customer(self, collected_values):
        ...
        partner = self.env['res.partner'].create(partner_create_values)
        country = partner.country_id
        if vat and partner._run_vat_test(vat, country, partner.is_company):
            partner.vat = vat
    ...
    def _import_ubl_invoice_add_currency(self, collected_values):
        ...
        currency_values['rate'] = currency._get_conversion_rate(
            ...
            company=company,
            date=collected_values['to_write']['invoice_date'],
        )
    ...
    def _import_ubl_invoice_add_allowance_charges_values(self, collected_values):
        tree = collected_values['tree']
    ...
    def _import_ubl_invoice_line_add_price_unit_quantity_discount(self, collected_values):
        ...
        total_allowance = sum(allowance['amount'] for allowance in collected_values['allowances'])
        ...
        subtotal = (line_extension_amount or 0.0) + total_allowance - total_charges

        if (
            line_extension_amount
            and invoiced_quantity
            and price_allowance_base_amount
        ):
            quantity = invoiced_quantity
            price_unit = price_allowance_base_amount
            discount_amount = (price_unit * quantity) - line_extension_amount
        elif (
            line_extension_amount
            and invoiced_quantity
            and not price_allowance_base_amount
        ):
            quantity = invoiced_quantity
            ...
            if price_amount:
                ...
                quantity = subtotal / price_unit
                discount_amount = (price_unit * quantity) - line_extension_amount
            else:
                price_unit = subtotal
                quantity = 1.0
                discount_amount = subtotal - line_extension_amount
        elif (
            not line_extension_amount
            and invoiced_quantity
            and price_allowance_base_amount
        ):
            price_unit = price_allowance_base_amount
            quantity = invoiced_quantity
            discount_amount = total_allowance - total_charges
        else:
            price_unit = 0.0
            quantity = 0.0
            discount_amount = 0.0

        collected_values['quantity'] = quantity
        collected_values['price_unit'] = abs(price_unit)
        gross_subtotal = (collected_values['price_unit'] * collected_values['quantity'])
        discount = collected_values['discount'] = (discount_amount * 100 / gross_subtotal) if gross_subtotal else 0.0

        to_write = collected_values['to_write']
        to_write['quantity'] = quantity
        to_write['price_unit'] = price_unit
        to_write['quantity'] = discount
        ...
        #         if price_amount_str and (price_amount := float(price_amount_str)):
    ...
    def _import_ubl_invoice_line_add_taxes_values(self, collected_values):
        ...
        # Percentage taxes.
        for node in line_tree.findall('.//{*}Item/{*}ClassifiedTaxCategory'):
            percentage = node.findtext('./{*}Percent')
            category_code = node.findtext('./{*}ID')
            ...
            global_tax_values = tax_total_values.get(tax_key)
            if not global_tax_values:
                continue

            tax_values = dict(global_tax_values)
    ...
    def _import_ubl_invoice_get_default_base_line_kwargs(self, collected_values):
        ...
        if partner := collected_values['partner_values'].get('partner'):
    ...
    def _import_ubl_invoice_line_get_product_base_line_kwargs(self, collected_values):
        ...
        if name := collected_values.get('name'):
            base_line_kwargs['_name'] = name
        if deferred_start_date := to_write.get('deferred_start_date'):
            base_line_kwargs['_deferred_start_date'] = deferred_start_date
        if deferred_end_date := to_write.get('deferred_end_date'):
            base_line_kwargs['_deferred_end_date'] = deferred_end_date
        return base_line_kwargs

    def _import_ubl_invoice_line_get_charge_base_line_kwargs(self, collected_values):
        charge = collected_values['charge']
        ...
        return {
            **self._import_ubl_invoice_get_default_base_line_kwargs(collected_values),
            ...
            '_name': f"{charge['reason_code'] or ''} {charge['reason']}",
        }
    ...
    def _import_ubl_invoice_add_base_lines(self, collected_values):
        ...
        for line_collected_values in lines_collected_values:
            ...
            for charge in lines_collected_values['charges']:
                # The charge has already been matched with a fixed tax.
                attempt_tax_values = charge.get('attempt_tax_values')
                if not attempt_tax_values or attempt_tax_values.get('tax'):
                    continue

                # Line representing a not matched fixed tax.
                base_line_kwargs = self._import_ubl_invoice_line_get_charge_base_line_kwargs({
                    **line_collected_values,
                    'charge': charge,
                })
                base_lines.append(AccountTax._import_ubl_invoice_get_fixed_tax_base_line_kwargs(
                    record=None,
                    **base_line_kwargs,
                ))
    ...
    def _ubl_import_invoice(self, invoice, file_data, new=False):
        ...
        # Extract information about allowance / charges.
        self._import_ubl_invoice_add_allowance_charges_values(collected_values)
