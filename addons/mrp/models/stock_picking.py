...
class StockPicking(models.Model):
    ...
    production_group_id = fields.Many2one(
        ...
        compute='_compute_production_group_id',
        search='_search_production_group_id',
    )
    ...
    @api.depends('move_ids.production_group_id')
    def _compute_production_group_id(self):
        for picking in self:
            picking.production_group_id = picking.move_ids.production_group_id[:1]

    def _search_production_group_id(self, operator, value):
        return [('move_ids.production_group_id', operator, value)]
