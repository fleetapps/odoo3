...
class MrpProduction(models.Model):
    ...
    def _compute_product_name(self):
        for production in self:
            ...
            if variant := production.product_id.product_template_attribute_value_ids._get_combination_name():
    ...
    def _compute_active_workcenter_ids(self):
        open_time_ids = self.env["mrp.workcenter.productivity"]._read_group(
            [("workorder_id", "in", self.workorder_ids.ids), ("date_end", "=", False)],
            ["workorder_id", "workcenter_id"],
        )
        workcenters_by_workorder = defaultdict(lambda: self.env['mrp.workcenter'])
        for (wo, wc) in open_time_ids:
            workcenters_by_workorder[wo] |= wc
        for production in self:
            workcenters = self.env['mrp.workcenter']
            for workorder in production.workorder_ids:
                workcenters += workcenters_by_workorder[workorder]
