...
class HrLeave(models.Model):
    ...
    def _l10n_in_find_linked_leave(self, start_date, public_holiday_dates, resource_calendar, leaves_by_date, reverse=False, check_extendability=False):
        step = -1 if reverse else 1
    ...
    def _l10n_in_prepare_sandwich_context(self):
        ...
        tz = ZoneInfo(self.env.company.tz or self.env.context.get("tz") or self.env.user.tz or "UTC")
        ...
        for company_id, recs in self.env['resource.calendar.leaves']._read_group(
            ...
            company_dates = {}
            for holiday in recs:
                local_start = _to_local_date(holiday.date_from)
                local_end = _to_local_date(holiday.date_to)
