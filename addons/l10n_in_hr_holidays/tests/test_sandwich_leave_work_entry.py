...
class TestSandwichLeaveWorkEntry(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.leave_type_test = cls.work_entry_type_leave
        cls.leave_type_test_1 = cls.work_entry_type_leave_1

    def get_work_entry_type_count(self, work_entry_create_vals, work_entry_type):
        if isinstance(work_entry_create_vals, dict):
            work_entry_create_vals = [work_entry_create_vals]
        return len([
            work_entry_vals
            for work_entry_vals in work_entry_create_vals
            if work_entry_vals.get('work_entry_type_id') == work_entry_type
        ])
    ...
    def check_work_entry_type_for_work_entry(self, work_entry_create_vals, work_entry_type):
        if isinstance(work_entry_create_vals, dict):
            work_entry_create_vals = [work_entry_create_vals]
        self.assertTrue(
            all(work_entry_vals.get('work_entry_type_id') == work_entry_type for work_entry_vals in work_entry_create_vals),
            'work entry type of work entry and leave should be same',
        )

    def _generate_and_search_work_entries(self, date_from, date_to):
        """Generate work entry values and keep only the requested employee/date range."""
        work_entry_vals = self.kohli_version.generate_work_entries(date_from.date(), date_to.date())
        return sorted([
            vals for vals in work_entry_vals
            if vals.get('employee_id') == self.kohli_emp
    ...
    def test_sandwich_leave_work_entry(self):
        ...
        sandwich_leave = self.Leave.create({
            ...
            'work_entry_type_id': self.leave_type_test.id,
    ...
    def test_sandwich_leave_with_different_leave_type(self):
        ...
        after_sandwich_leave = self.Leave.create({
            'name': 'Test Leave 1',
            'employee_id': self.kohli_emp.id,
            'work_entry_type_id': self.leave_type_test_1.id,
