    ...
    def _add_supplier_to_product(self):
        ...
        for line in self.order_line:
            ...
            already_seller = line.product_id.seller_ids.filtered(
                lambda s: s.partner_id in (partner | self.partner_id) and
                (s.product_id == line.product_id if line.product_id.product_variant_count > 1 else True)
            )
            if line.product_id and not already_seller and len(line.product_id.seller_ids) <= 10:
                ...
                if not seller_vals:
                    ...
                    current = seller_vals[0]
                    if not current.get('product_id') or current.get('product_id') == supplierinfo.get('product_id'):
                        continue
                    same_supplier = (
                        len(seller_vals) == 1 and
                        all(current.get(field) == supplierinfo.get(field) for field in ('price', 'delay'))
                    )
                    if same_supplier:
                        # merge into template-level supplier (remove variant specificity)
                        current.pop('product_id', None)
                    else:
                        seller_vals.append(supplierinfo)
        for template_id, vals in vals_by_template.items():
            already_seller = template_id.seller_ids.filtered(lambda s: s.partner_id in (partner | self.partner_id) and not s.product_id)
            if already_seller and len(vals['seller_ids']) == 1 and not vals['seller_ids'][0].get('product_id'):
                continue
