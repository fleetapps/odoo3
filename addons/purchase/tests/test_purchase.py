...
class TestPurchase(AccountTestInvoicingCommon):
    ...
    def test_variant_level_pricelist_created_when_prices_differ(self):
        ...
        at the template level when all variants share the same conditions, and at the
        variant level when price, UoM, or lead time differs between variants.
        """
        ...
        self.assertRecordValues(product_sofa.seller_ids, [
            {'product_id': False, 'partner_id': self.partner_a.id, 'price': 200, 'min_qty': 1, 'delay': 0},
            {'product_id': variants[0].id, 'partner_id': self.partner_b.id, 'price': 100, 'min_qty': 1, 'delay': 1},
            {'product_id': variants[1].id, 'partner_id': self.partner_b.id, 'price': 110, 'min_qty': 1, 'delay': 2},
        ])
