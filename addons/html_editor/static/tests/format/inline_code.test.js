...
test("should paste Odoo editor html as plain text inside inline code", async () => {
    await testEditor({
        ...
        stepFunction: async (editor) => {
            const clipboardData = new DataTransfer();
            clipboardData.setData("text/plain", "Hello Odoo");
            clipboardData.setData(
                "application/vnd.odoo.odoo-editor",
                `<p class="o_paragraph">Hello <strong>Odoo</strong></p>`
            );
            const pasteEvent = new ClipboardEvent("paste", {
                clipboardData,
                bubbles: true,
        ...
        contentAfter: `<p>ab<code class="o_inline_code">Hello Odoo[]</code>cd</p>`,
    });
});
