...
export class InlineCodePlugin extends Plugin {
    ...
    resources = {
        ...
        /** Predicates */
        paste_text_predicates: (selection) => {
            const caretNode =
                selection.direction === DIRECTIONS.RIGHT
                    ? selection.anchorNode
                    : selection.focusNode;
            return Boolean(closestElement(caretNode, "code.o_inline_code"));
        },
        powerbox_visibility_predicates: (node) => !closestElement(node, "code.o_inline_code"),
