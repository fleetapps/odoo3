...
class AccountMove(models.Model):
    ...
    def _group_by_error_code(self):
        self.ensure_one()
        if self.move_type == 'out_refund' and not self.l10n_pk_edi_refund_reason:
            return (
                'danger',
    ...
    def _l10n_pk_edi_tax_groups(self, company):
        ...
        sales_tax_group_ids = [group for ref in sales_tax_group_ids if (group := IrModelData._xmlid_to_res_model_res_id(f'account.{company.id}_{ref}')[1])]
        sales_tax_group = self.env['account.tax.group'].browse(sales_tax_group_ids)
        withhold_tax_group = self.env['account.chart.template'].with_company(company).ref('tax_group_pk_wt')
    ...
    def _get_l10n_pk_edi_line_details(self, move_lines):
        ...
        for line in invoice_lines:
            ...
            if not product.l10n_pk_edi_hs_code:
                raise ValidationError("HS Code is required for FBR E-Invoice.")
            ...
            rate_value = f"{int(sales_tax_rates[0]) if sales_tax_rates[0] == int(sales_tax_rates[0]) else sales_tax_rates[0]}%" if sales_tax_rates else "0%"
            ...
            item_payload = {
                ...
                'salesTaxWithheldAtSource': _round(tax_values.get('withhold_tax_total')),
                'extraTax': _round(tax_values.get('extra_tax_total')),
                'furtherTax': 0.0,
                'fedPayable': 0.0,
                'discount': _round(discount_amount),
                'saleType': sale_type_selection.get(product.l10n_pk_edi_transaction_type, 'Goods at standard rate'),
            }
    ...
    def _l10n_pk_edi_generate_invoice_json(self):
        ...
        is_buyer_registered = buyer.l10n_pk_edi_is_fbr_registered
        ...
        json_payload = {
            ...
            'buyerRegistrationType': 'Registered' if is_buyer_registered else 'Unregistered',
            'buyerNTNCNIC': buyer.vat if is_buyer_registered else '0000000000000',
        ...
        if self.move_type == 'out_refund' and self.reversed_entry_id and self.reversed_entry_id.l10n_pk_edi_status == 'sent':
