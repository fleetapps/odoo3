...
class AccountMove(models.Model):
    ...
    def button_request_cancel(self):
        for move in self:
            if move._l10n_es_edi_sii_need_cancel_request():
                return move._l10n_es_edi_cancel_invoice()
    ...
    def button_draft(self):
        ...
        self.write({
            'l10n_es_edi_sii_state': False,
            'l10n_es_edi_csv': False,
            'l10n_es_edi_sii_error': False,
        })
        return res
    ...
    def _post(self, soft=True):
        ...
        self.filtered(lambda m: m.l10n_es_edi_is_required and not m.l10n_es_edi_sii_state).l10n_es_edi_sii_state = 'to_send'
        return res

    def _l10n_es_edi_sii_need_cancel_request(self):
        self.ensure_one()
        return self.l10n_es_edi_is_required and self.l10n_es_edi_sii_state == 'sent'
    ...
    def _send_l10n_es_sii_document(self, cancel=False):
        ...
        if document.state == 'rejected':
            self.l10n_es_edi_sii_error = document.response_message
        elif document.state in ('accepted', 'accepted_with_errors'):
            self.l10n_es_edi_sii_state = 'sent'
            self.l10n_es_edi_csv = document.csv
        elif document.state == 'cancelled':
            self.l10n_es_edi_sii_state = 'cancelled'

        if document.state in ('accepted', 'accepted_with_errors', 'cancelled'):
            self.l10n_es_edi_sii_document_ids.filtered(lambda d: d.state == 'rejected').unlink()
    ...
    def _l10n_es_edi_get_invoices_tax_details_info(self, filter_invl_to_apply=None):
        ...
        for tax_values in tax_details['tax_details'].values():
            ...
            if self.is_sale_document():
                ...
                if tax_values['l10n_es_type'] not in ['retencion', 'recargo']:
                    base_amount = sign * tax_values['base_amount']
