...
class PaymentProvider(models.Model):
    ...
    def write(self, vals):
        ...
        if 'bank_account_id' in vals:
            for provider in self.filtered(
                lambda provider: provider.custom_mode == 'wire_transfer',
            ):
    ...
    def _recompute_pending_msg(self):
        ...
        if account:
            account_fields = [
                (self.env._("Beneficiary"), account.holder_name),
                (self.env._("Bank Account"), account.display_name),
            ]

            account_details = ''.join(
                f'<div class="mt-3">'
                f'<b>{label}: </b>'
                f'<span class="o_payment_instructions_box">{field}</span>'
                f'</div>'
                for label, field in account_fields
            )
